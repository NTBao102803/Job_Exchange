...
const RecruiterPageView = () => {
  ...
  const url = `http://localhost:8080/ws-comments?token=${encodeURIComponent(cleanToken)}`;
  ...
  client.onConnect = () => {
    console.log("✅ WebSocket connected! Subscribing to:", `/topic/comments/${recruiter.authUserId}`);
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      await submitCandidateComment({ employerId: recruiter.authUserId, content: newComment.trim(), authorName:user.fullName, rating: commentRating === 0 ? null : commentRating, userId:user.id });
      setNewComment("");
    } catch {
  ...
  const handleSubmitReply = async (parentId, content) => {
    ...
    try {
      await submitCandidateComment({ employerId: recruiter.authUserId, content: content.trim(), authorName:user.fullName,userId:user.id, parentId });
    } catch {
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow p-5 border border-gray-200">
              ...
              <div className="flex justify-end items-center gap-2 mb-3">
                <span className="text-sm text-gray-600">Hãy đánh giá sao:</span>
                <div className="flex items-center gap-1">
                    {[1,2,3,4,5].map((i) => (
                    <Star
                        key={i}
